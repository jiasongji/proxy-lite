...
### 1. 服务器 B：Shadowsocks 落地（一键复制）

把占位符替换为你的域名、服务器显示地址、端口和密码后，在服务器 B 的 root 用户下执行：
...
### 2. 服务器 A：用户入口，固定经 B 出口（一键复制）

把占位符替换为服务器 A 的域名、证书路径、入口端口和服务器 B 的 Shadowsocks 信息后，在服务器 A 的 root 用户下执行：
